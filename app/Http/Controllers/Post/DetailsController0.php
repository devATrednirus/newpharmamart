...
class DetailsController extends FrontController
{
	...
	public function sendMessage($postId, SendMessageRequest $request)
	{
		...
		$message =  Message::withTrashed()->whereNull('message_id')->whereNull('quick_message_id')->where('from_user_id',$user_id)->where('post_id',(!empty($post->id) ? $post->id : 0) )->whereNull('drugs_license')->first();
		...
		$message->post_id = (!empty($post->id) ? $post->id : 0 );
		$message->from_user_id = $user_id;
		$message->to_user_id = (!empty($post->user_id) ? $post->user_id : 0);
		$message->to_name = (!empty($post->user->first_name) ? $post->user->first_name : ' ');
		$message->to_email = (!empty($post->user->email) ? $post->user->email : ' ');
		$message->to_phone = (!empty($post->user->phone) ? $post->user->phone : '  ');
		$message->subject = (!empty($post->title) ? $post->title : ' ');
		$message->looking_for = (!empty($post->category->name) ? $post->category->name : ' ');
		...
		$message->category_id = (!empty($post->category->id) ? $post->category->id : 1);
	...
	public function updateQueryMessage(Request $request)
	{
		...
		/* if(!auth()->check()){
			return response()->json([
                'code' => 100,
                'message' => "Please wait for login request to complete."
            ],422);
		} */
	...
	/* 	$validatedData = $request->validate([
		...
		//$message =  Message::withTrashed()->whereNull('message_id')->whereNull('quick_message_id')->where('from_user_id',auth()->user()->id)->find($data['query_id']);
		$message =  Message::withTrashed()->whereNull('message_id')->whereNull('quick_message_id')->find($data['query_id']);
		...
		if(!empty($data['location_for_franchise'])) {
		...
		if(!empty($data['specific_query']) || $data['specific_query']!=""){
