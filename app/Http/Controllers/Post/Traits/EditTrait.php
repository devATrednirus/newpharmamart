...
trait EditTrait
{
    ...
    public function postUpdateForm($postIdOrToken, PostRequest $request)
    {
        ...
        if (getSegment(2) == 'create') {
            ...
            $post = Post::with('user')->withoutGlobalScopes([VerifiedScope::class, ReviewedScope::class])
				->where('id', session('tmpPostId'))
				->where('tmp_token', $postIdOrToken)
				->first();
        } else {
       ...
       /* $user = User::withoutGlobalScopes([VerifiedScope::class])->find(auth()->user()->id);
		...
		if (config('settings.single.posts_review_activation')) {
			if (
                ...
                md5($post->short_description) != md5($request->input('short_description'))
			) {
				$post->reviewed = 0;
			}
		}
        ...
        if($post->reviewed==0){
            flash("Your ad has been updated and currently under review")->success();
        }
        else{
