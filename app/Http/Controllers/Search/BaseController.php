...
class BaseController extends FrontController
{
    ...
    public function commonQueries()
    {
        ...
        $cats = Cache::remember($cacheId, $this->cacheExpiration, function () {
            $cats = Category::where('is_hidden','0')->trans()->orderBy('lft')->get();
            return $cats;
        });
        if ($cats->count() > 0) {
        ...
        if (config('settings.listing.left_sidebar')) {
            ...
            $sql = 'SELECT sc.id, c.parent_id, count(*) as total' . '
				...
				INNER JOIN ' . DBTool::table('categories') . ' as sc ON sc.id=a.category_id AND sc.active=1 AND sc.is_hidden=0
				INNER JOIN ' . DBTool::table('categories') . ' as c ON c.id=sc.parent_id AND c.active=1 AND c.is_hidden=0
            ...
            $sql1 = 'SELECT c.id as id, count(*) as total' . '
                FROM ' . DBTool::table('posts') . ' as a
                INNER JOIN ' . DBTool::table('categories') . ' as c ON c.id=a.category_id AND c.active=1  AND c.is_hidden=0
                WHERE a.country_code = :countryCode AND (a.verified_email=1 AND a.verified_phone=1) AND a.archived!=1 AND a.deleted_at IS NULL
                GROUP BY c.id';
           ...
           // $countCatPosts = DB::select(DB::raw($sql), $bindings);
            ...
            $cities = Cache::remember($cacheId, $this->cacheExpiration, function () use ($limit) {
                $cities = City::currentCountry()->whereHas('posts')->take($limit)->orderBy('population', 'DESC')->orderBy('name')->get();
                return $cities;
            });
