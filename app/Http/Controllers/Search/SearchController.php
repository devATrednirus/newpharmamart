...
*/
        ...
        if($this->sUser->about_us==""){
          ...
          //dd(' '.$this->sUser->city->subAdmin1->name);


            $cmp_groups = ucwords(implode(", ",ProductGroup::where('user_id',$this->sUser->id)->orderBy('name')->pluck('name')->take(5)->toArray()));

            $subadminname = empty($this->sUser->city->subAdmin1->name) ? '' : $this->sUser->city->subAdmin1->name;

            $this->sUser->about_us= "<p><strong>".$this->sUser->name.'</strong>'.(!empty($this->sUser->businessType)?" is <strong>".$this->sUser->businessType->name.'</strong>':'').(!empty($this->sUser->city)?" in ".$this->sUser->city->name.($this->sUser->city->name!=$subadminname?", ".$subadminname:''):'').".".($this->sUser->establishment_year?" Established in Year <strong>".$this->sUser->establishment_year."</strong>.":'').'<strong>'.$this->sUser->name."</strong> is Running under the Guidance of ".($this->sUser->ceo_first_name?" <strong>".ucwords($this->sUser->ceo_first_name).($this->sUser->ceo_last_name?" ".ucwords($this->sUser->ceo_last_name):'').'</strong>':' Our CEO').($this->sUser->no_employees?" with <strong>".$this->sUser->no_employees.'</strong> team members.':'').($this->sUser->annual_turnover?" Our Company has annual turnover <strong>".$this->sUser->annual_turnover.'</strong>.':'').($cmp_groups?" We are offering <strong>".$cmp_groups.'</strong>.':'')."</p>";
                ...
                // $keywords=[];
                 $product_groups=[];
                    foreach ($posts as $key => $post) {
                            ...
                            $catNestedIds = (object)[
                                'parentId' => (empty($post->category)?0:$post->category->parent_id),
                                'id'       => (empty($post->category)?0:$post->category->tid),
                            ];
