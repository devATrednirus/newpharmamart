...
trait TitleTrait
{
	...
	public function getTitle()
	{
		...
		if (request()->filled('q')) {
			$title .= ' ' . t('for') . ' ';
			$title = rawurldecode(request()->get('q')).' - Rednirus Mart';
			return $title;
		}
			...
			if (isset($this->cat) && !empty($this->cat)) {
				...
				if (isset($this->isSubCatSearch) && $this->isSubCatSearch) {
					if (isset($this->subCat) && !empty($this->subCat)) {
 						...
 						if($this->subCat->keywords){
							 return $this->subCat->keywords;
						}
					}

				}else if($this->cat->keywords){
					 return $this->cat->keywords;
				}
