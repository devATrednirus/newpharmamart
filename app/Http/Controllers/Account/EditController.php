...
class EditController extends AccountBaseController
{
	...
	public function updateCompanyDetails(CompanyRequest $request)
	{
		...
		$input = $request->only(['name','establishment_year','ceo_first_name','ceo_last_name','business_type','owner_type','no_employees','annual_turnover','website','about_us','why_us','our_product']);
		foreach ($input as $key => $value) {
			...
			$user->{$key} = $value;
                         ...
                         $user->corporate_video = $request->input('corporate_video');
	...
	public function updatePhoto($userId, Request $request)
	{
		...
		$destinationPath = 'avatars/in/' . $user->id;
		//$filename = $file->getClientOriginalName();
		// Storage::put('temp', File::get($file->getrealpath()));
		$file->move(base_path().'/public/storage/'.$destinationPath, last(explode('/', $user->photo)));
