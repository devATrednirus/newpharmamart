...
class PostsController extends AccountBaseController
{
	...
	public function getMyPosts($postId = null)
	{
		...
		$this->perPage = session('numberofrows', 10);
		$data['posts'] = $this->myPosts->paginate($this->perPage);
		$data['numberofrows'] = $this->perPage;
	 ...
	 * @return \Illuminate\Contracts\View\Factory|\Illuminate\Http\RedirectResponse|\Illuminate\View\View
	 */
	public function getMyGroups($groupId = null)
	{
		$pagePath = 'my-groups';
		
	 
		$data = [];
		$data['groups'] = $this->myGroups->paginate($this->perPage);
		
		// Meta Tags
		MetaTag::set('title', t('My Groups'));
		MetaTag::set('description', t('My groups on :app_name', ['app_name' => config('settings.app.app_name')]));
		
		view()->share('pagePath', $pagePath);
		$data['numberofrows'] = session('numberofrows', 10);
		return view('account.groups', $data);
	}
	 ...
	 * @return \Illuminate\Contracts\View\Factory|\Illuminate\Http\RedirectResponse|\Illuminate\View\View
	 */
	public function getDivisions($groupId = null)
	{
		$pagePath = 'divisions';
		
	 
		$data = [];
		$data['divisions'] = $this->divisions->paginate($this->perPage);
		
		// Meta Tags
		MetaTag::set('title', 'Divisions');
		//MetaTag::set('description', t('My divisions on :app_name', ['app_name' => config('settings.app.app_name')]));
		
		view()->share('pagePath', $pagePath);
		
		return view('account.divisions', $data);
	}
	...
	public function destroy($pagePath, $id = null)
	{
		...
		elseif ($pagePath == 'divisions') {

			$savedDivisions = Division::where('user_id', auth()->user()->id)->whereIn('id', $ids);
			if ($savedDivisions->count() > 0) {
				$nb = $savedDivisions->delete();
			}
		...
		if ($nb == 0) {
			...
			$count = count($ids);
			if ($count > 1) {
				$message = t("x :entities has been deleted successfully.", ['entities' => $pagePath, 'count' => $count]);
			} else {
				$message = t("1 :entity has been deleted successfully.", ['entity' => $pagePath]);
			}
			flash($message)->success();
		}
	...
	public function repostproduct(HttpRequest $request)
	{
		
		$emp=implode(',',@$request->entries);
		$array=array(
		'archived'=>0,
		'archived_at'=>''
		);
		DB::table('posts')->whereIn('posts.id',explode(",", $emp))->update($array);
		$message ="Product Repost Successfully";
			
			flash($message)->success();
		return redirect(config('app.locale') . '/account/my-posts');
	}
	public function numberofrows(HttpRequest $request)
	{
		
 		if(isset($request->numberofrows)){
			$this->perPage =	$request->numberofrows;
			session(['numberofrows' => $request->numberofrows]);
 		} else {
	
			$this->perPage = session('numberofrows', 10);
		}
		return redirect(config('app.locale') . '/account/'.$request->redirect_page);

	}
	public function archiveproduct(HttpRequest $request)
	{
		
		
		//echo $count=sizeof(@$request->entries);
		//exit;
		if(isset($request->numberofrows)){
			$this->perPage =	$request->numberofrows;
			session(['numberofrows' => $request->numberofrows]);
		...
		DB::table('posts')->whereIn('posts.id',explode(",", $emp))->update($array);
		$message ="Product Offline Successfully";
			
			flash($message)->success();
		return redirect(config('app.locale') . '/account/archived');
		}
	}
	function addfavourite(HttpRequest $request)
	{
		...
		foreach($data as $row)
		{
			...
			DB::table('saved_posts')->insert($arr);
		}
		$message ="Product Successfully Added To favourite List ";
			
			flash($message)->success();
		return redirect(config('app.locale') . '/account/favourite');
	}
	public function removefavourite(HttpRequest $request)
	{
		$data=@$request->entries;
		foreach($data as $row)
		{
			DB::table('saved_posts')->where(['post_id'=>$row])->delete();
		}
