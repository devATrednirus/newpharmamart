...
class PostController extends FrontController
{
    ...
    public function saveGroup(Request $request)
    {
        ...
        if (auth()->check()) {
            ...
            $savedGroup = ProductGroup::where('user_id', auth()->user()->id)->where('slug', slugify($name));
            if ($savedGroup->count() > 0) {
                 
                return response()->json(['error' => ['message' => "Error. group alread exist."], 404]);
                ...
                $savedGroup = new ProductGroup($savedGroupInfo);
                $savedGroup->save();

                return response()->json($savedGroup, 200, [], JSON_UNESCAPED_UNICODE);
        ...
        return response()->json([], 402, [], JSON_UNESCAPED_UNICODE);
    }
    /**
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
