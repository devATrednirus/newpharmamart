...
class LocationController extends FrontController
{
			...
			$cities = City::countryOf($countryCode)->with(['subAdmin1', 'subAdmin2'])
				->where(function ($builder) use ($query) {
					...
					$builder->orWhere(function($builder) use ($query){
						$builder->whereHas('subAdmin1', function ($builder) use ($query){
						    $builder->where('name', 'LIKE', $query . '%');
						});
					});
					// $builder->orWhere(function($builder) use ($query){
					// 	$builder->whereHas('subAdmin2', function ($builder) use ($query){
					// 	    $builder->where('name', 'LIKE', $query . '%');
					// 	});
					// });
	...
	public function getSelectedCity($countryCode, $cityId)
	{
		...
		if (!empty($city)) {
			...
			if (isset($city->subAdmin1) && !empty($city->subAdmin1)) {
				$text .= ', ' . $city->subAdmin1->name;
				$state = $city->subAdmin1->name;
			}
			 
			$cityArr = ['id' => $city->id, 'text' => $text,'state'=>$state];
		} else {
			$cityArr = ['id' => 0, 'text' => t('Select a city', [], 'global', request()->get('languageCode')),'state'=>''];
		}
	...
	public function getAdmin1WithCities($countryCode, Request $request)
	{
		...
		foreach ($cities as $col) {
			...
			$html .= '<ul class="list-link list-unstyled skin-blue">';
			$j = 0;
			foreach ($col as $city) {
				if ($i == 0 && $j == 0) {
