	...
	public function setup()
	{
		...
		*/
		...
		$this->xPanel->addColumn([
			...
			'label'         => 'Sub category',
			'type'          => 'model_function',
			'function_name' => 'getSubCategoryHtml',
		]);
		/*$this->xPanel->addColumn([
			'name'          => 'country_code',
		...
		foreach ($entries as $entry) {
			...
			if (!empty($subEntries)) {
				foreach ($subEntries as $subEntrie) {
					...
					$subEntriesNew = Category::trans()->where('parent_id', $subEntrie->id)->orderBy('lft')->get();
					if (!empty($subEntriesNew)) {
						foreach ($subEntriesNew as $subEntrieNew) {
							$tab[$subEntrieNew->tid] = "------| " . $subEntrieNew->name;
						}
					}
				}
			}
