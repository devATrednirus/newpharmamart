...
function check404url($url){
	$arr =explode('/',$url);
//print_r($arr); die;
	$search_category=in_array("category", $arr);
	$search_detail=in_array("detail", $arr);
	$search_tags=in_array("tag", $arr);
	$search_index_php=in_array("index.php", $arr);

	if($search_category || $search_index_php){
	$urlcount=count($arr)-1;
	$last_value=slugify($arr[$urlcount]);

		$data=DB::table('categories')->select('slug','active')->where('slug','like','%'.$last_value.'%')->where('active','=','1')->get();
...
if($data->active==1){
	...
	header("location: ".$lurl."/category/".$data->slug);
	exit();
}else{
	...
	}}elseif($search_detail){
	$urlcount=count($arr)-1;
	$last_value=slugify($arr[$urlcount]);
	$url_decode=str_replace('-',' ',$last_value);
		$data=DB::table('posts')->select('id','user_id','title','reviewed','deleted_at')->where('title','like','%'.$url_decode.'%')->get();
		$resultCount = $data->count();
		if($resultCount>0){
		foreach ($data as $data) {
if($data->reviewed==1 && $data->deleted_at==''){
	//return \Redirect::to(url("/").'/detail/'.slugify($data->title), 301);
	header("HTTP/1.1 301 Moved Permanently");
	header("location: /detail/".$data->id.'/'.slugify($data->title));
	exit();
}else{
	$dataselller=DB::table('users')->select('username')->where('id','=',$data->user_id)->get();
	$resultCount = $dataselller->count();
	if($resultCount>0){
	foreach ($dataselller as $dataselller) { //die(slugify($dataselller->username));
	//return \Redirect::to(url("/").$dataselller->username, 301);
	header("HTTP/1.1 301 Moved Permanently");
	header("location: /".$dataselller->username);
	exit();
}
}else{
	header("HTTP/1.1 301 Moved Permanently");
	...
	$urlcount=count($arr)-1;
	$last_value=$arr[$urlcount];
		$data=DB::table('posts')->select('id','user_id','title','reviewed')->where('tags','like','%'.$last_value.'%')->get();
		$resultCount = $data->count();

		if($resultCount>0){
		foreach ($data as $data) {
if($data->reviewed==1){
	//return \Redirect::to(url("/").'/tag/'.slugify($last_value), 301);
	header("HTTP/1.1 301 Moved Permanently");
	header("location: /tag/".slugify($last_value));
	exit();
}else{
	$dataselller=DB::table('users')->select('username')->where(['id'=>$data->user_id])->first();
	//return \Redirect::to(url("/").$dataselller->username, 301);
	header("HTTP/1.1 301 Moved Permanently");
	header("location: /".$dataselller->username);
	exit();
	...
	header("location: /");
	exit();
	}}elseif(strpos($url, 'company/product/') !== false){
	$urlcount=3;
	$last_value=slugify($arr[$urlcount]);

		$data=DB::table('users')->select('username')->where('username','like','%'.$last_value.'%')->where('user_type_id','=','1')->where('blocked','!=','1')->where('package_id','!=','')->get();
		$resultCount = $data->count();
	...
	print_r($arr);
	echo $urlcount=count($arr)-1;
	echo $last_value=slugify($arr[$urlcount]);
...
function getCities()
{
	...
	// \DB::enableQueryLog();
	$cities = \App\Models\City::with('subAdmin1')->orderBy('name','asc')->get();
		/*dump(\DB::getQueryLog());
	 dd($categories);
	*/
	if ($cities->count() > 0) {
		foreach ($cities as $city) {
			...
			$arr[$city->id] = $city->name.(($city->subAdmin1)?", ".$city->subAdmin1->name:"");
		}
	}
 ...
 * @return array
 */
function getCategories($type="main")
{
	$arr = [];

	// \DB::enableQueryLog();
	$categories = \App\Models\Category::where(function($q)use($type){
		if($type=="main"){
			$q->where('parent_id','0')->has('children.posts');
		}
		else{
			$q->where('parent_id','!=','0')->has('posts');
		}
	})->get();
		/*dump(\DB::getQueryLog());
	 dd($categories);
...
function resize($pathFromDb, $type = 'big')
{
	...
	// Thumb file name
	$thumbFilename = 'thumb-new-' . $size . '-' . $filename;
	...
	if (\Storage::exists($filepath . $thumbFilename)) {
		...
		try {
			...
			if (in_array($type, ['logo'])) {
				...
				// Fit small pictures
				$image = \Image::make(\Storage::get($pathFromDb))->fit($width, $height)->encode("webp", config('larapen.core.picture.quality', 100));
			}
		} catch (\Exception $e) {
			return \Storage::url($pathFromDb) . getPictureVersion();
		}
...
function getLoginField($value)
{
	...
	if (filter_var($value, FILTER_VALIDATE_EMAIL)) {
		$field = 'email';
	} else if (is_array($value) || preg_match('/^((\+|00)\d{1,3})?[\s\d]+$/', $value)) {
		$field = 'phone';
	}
...
function getRawBaseUrl()
{
	...
	// Get the Current URL
	$currentUrl = ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off') ? 'http://' : 'http://') . $httpHost . strtok($requestUri, '?');
