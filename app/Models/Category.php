...
class Category extends BaseModel
{
	...
	protected $cast = ['integer'=>'is_hidden','integer'=>'featured','integer'=>'in_footer'];
	...
	*/
	...
	{
		if (!isset($this->is_hidden)) return false;

        return checkboxDisplay($this->is_hidden);
	}
	...
	public function getFeaturedHtml()
	{
        ...
        return checkboxDisplay($this->featured);
	}


	public function getExludeLocationHtml()
	{
		if (!isset($this->exclude_location)) return false;

        return checkboxDisplay($this->exclude_location);
	}
	...
	public function subCategoriesBtn($xPanel = false)
	{
		...
		if ($this->parent_id == 0 || $this->parent->parent_id == 0) {
	...
	*/
	...
	{
		return $this->hasManyThrough(Post::class, Category::class, 'id', 'category_id');
	}
	...
	public function children5()
	{
		return $this->hasMany(Category::class, 'parent_id', 'translation_of')->where('translation_lang', config('app.locale'))->take(5);
	}
	...
	public function setPictureAttribute($value)
	{
		...
		try {
			if (fileIsUploaded($value)) {
				...
				if($this->parent_id=="0"){
					$image = Image::make($value)->resize(350, 500, function ($constraint) {
