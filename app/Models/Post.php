...
class Post extends BaseModel implements Feedable
{
	...
	*/
	...
	public function routeNotificationForMail()
	{
		...
		//return "munish.malhotra07@gmail.com";

		if($this->user && isset($this->user->email_to_send)){

			return $this->user->email_to_send;
	...
	public function getIdHtml()
	{
		...
		return '<input name="entryId[]" type="checkbox" value="'.$this->id.'" class="dt-checkboxes">';
	}
	...
	public function getPictureHtml()
	{
		...
		if ($this->pictures->count() > 0) {
			foreach ($this->pictures as $picture) {
				...
				$out = '<img src="' . str_replace('storage','storage/app',resize($picture->filename, 'small')) . '" data-toggle="tooltip" title="' . $this->title . '"' . $style . '>';
				break;
			}
		} else {
			// Default picture
			$out = '<img src="' . str_replace('storage','storage/app',resize(config('larapen.core.picture.default'), 'small')) . '" data-toggle="tooltip" title="' . $this->title . '"' . $style . '>';
		}
	...
	public function getUserNameHtml()
	{
		if (isset($this->user) and !empty($this->user)) {
			...
			$tooltip = ' data-toggle="tooltip" title="' . $this->contact_name . '"';

			return '<a href="' . $url . '"' . $tooltip . '>' . $this->user->name . '</a>';
		} else {
	...
	{
		if (isset($this->category) and !empty($this->category) && $this->category->parent) {
	...
	*/
	...
	{
		return $this->hasOne(Picture::class, 'post_id')->orderBy('position')->orderBy('id', 'DESC');
	}
