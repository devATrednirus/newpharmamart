...
trait Query
{
	...
	public function count($lang = null)
	{
		...
		if(isset($this->with_trashed) && $this->with_trashed==true){
			if (property_exists($this->model, 'translatable')) {
				$countEntries = $this->query->withTrashed()->where('translation_lang', $lang)->count();
			} else {
				$countEntries = $this->query->withTrashed()->count();
			}
