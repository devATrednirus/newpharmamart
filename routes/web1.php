...
Route::group([
	...
	Route::group([
		...
		//CRUD::resource('blocked-locations/{state_id}/show', 'BlockedLocationController');
...
Route::group([
	...
	// SEO
	if(!isset($_SERVER['SERVER_NAME']) || (preg_match('/pharmafranchisemart.com/',$_SERVER['SERVER_NAME']) || preg_match('/127.0.0.1/',$_SERVER['SERVER_NAME']) ) ){
...
Route::group([
	...
	Route::group(['middleware' => ['web']], function ($router) { //, 'install.checker'
		// HOMEPAGE
		...
		if(!isset($_SERVER['SERVER_NAME']) || (preg_match('/pharmafranchisemart.com/',$_SERVER['SERVER_NAME']) || preg_match('/127.0.0.1/',$_SERVER['SERVER_NAME']) ) ){
			...
			Route::post(LaravelLocalization::transRoute('routes.search-cat'), 'Search\CategoryController@index');
