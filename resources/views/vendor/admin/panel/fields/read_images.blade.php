...
<div @include('admin::panel.inc.field_wrapper_attributes') >
	...
	@if (!empty($entity_model) && !$entity_model->isEmpty())
        ...
        @if ($entity_model->count() < $posts_pictures_number)
            <hr><br>
            <a target="_blank" href="{{ admin_url('pictures/create?post_id=' . request()->segment(3)) }}" class="btn btn-xs btn-default"><i class="fa fa-edit"></i> {{ trans('admin::messages.add') }} {{ trans('admin::messages.picture') }}</a><br><br>
        @endif
	@else
