...
{
	$url=$appurl.'/admin/import_category';
}
...
{
	$url=$appurl.'/admin/import_product';
}
if(Request::path()=='admin/payments')
{
        ...
        jQuery(document).ready(function($) {
            ...
            var table = $("#crudTable").DataTable({
                    ...
                    "ajax": {
						...
						//"url": "<?=$url?>search?<?=Request::getQueryString()?>",
                        "type": "POST",
                      //  data: {
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
	  ...
	  <form method="post" id="laravel-ajax-file-upload"  enctype="multipart/form-data">
		...
		<center><button class="btn btn-primary" id="importing">Import</button> @if(Request::path()=='admin/categories')
		<a href="{{url('admin/downloadcategorycsv')}}" class="btn btn-danger"> <i class="fa fa-download"></i> Download Sample </a>
	   @endif
	    @if(Request::path()=='admin/users')
		<a href="{{url('admin/downloadusercsv')}}" class="btn btn-danger"> <i class="fa fa-download"></i> Download Sample </a>
		@endif</center>
		</div>
		</div>
		</form>
...
<div id="ImportCity" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
	  ...
	  <form method="post" id="laravel-ajax-file-upload-city"  enctype="multipart/form-data">
        <div class="row">
		<div class="col-md-12">
		...
		<center><button class="btn btn-primary" id="importing-city">Import</button>

		<a href="{{url('admin/downloadcitycsv')}}" class="btn btn-danger"> <i class="fa fa-download"></i> Download Sample </a>
		</center>
		</div>
		</div>
		</form>
...
function categoryexport()
{
	 ...
	 $('#bulkActionForm').attr('action', '<?=$appurl?>/admin/download');
   $('#bulkActionForm').submit();
}

function userexport(a)
{
	//alert(a);
	$("#usertype").val(a);
	$('#bulkActionForm').attr('action', '<?=$appurl?>/admin/userexcelexport');
   $('#bulkActionForm').submit();
}
...
$('#laravel-ajax-file-upload-city').submit(function(e) {
            e.preventDefault();
            $('#importing-city').text('Uploading...');
            var formData = new FormData(this);
            $.ajax({
                type: 'POST',
                url: "{{ url('admin/import_city')}}",
                data: formData,
                cache: false,
                contentType: false,
                processData: false,
