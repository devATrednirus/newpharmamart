...
if ($post->category) {
    if ($post->category->parent_id == 0) {
    	...
    	if($post->category->children->count()>0){
								...
								<form class="form-horizontal" id="postForm" method="POST" action="{{ url()->current() }}" enctype="multipart/form-data">
									...
									<fieldset>
										...
										<div class="form-group row required">
											<label class="col-md-3 col-form-label{{ $parentIdError }}"> Pharmafranchisemart Category <sup>*</sup></label>
											<div class="col-md-8">
												<select name="parent_id" id="parentId" class="form-control selecter{{ $parentIdError }}">
													...
													<option value="0" data-type=""
															...
															@endif
													>
														Select a Pharmafranchisemart category
													</option>

													@foreach ($categories as $mcat)
														<optgroup label="{{ $mcat->name }}">
															@foreach ($mcat->children as $cat)
										...
										<div id="subCatBloc" class="form-group row required">
											<label class="col-md-3 col-form-label{{ $categoryIdError }}">Pharmafranchisemart Sub-Category <sup>*</sup></label>
											<div class="col-md-8">
												<select name="category_id" id="categoryId" class="form-control selecter{{ $categoryIdError }}">
													<option value="0"
															...
															@endif
													>
														Select a Pharmafranchisemart sub-category
													</option>
												</select>
											</div>
										</div>
										...
										--}}
										<!-- group_id -->
										<?php $groupIdError = (isset($errors) and $errors->has('group_id')) ? ' is-invalid' : ''; ?>
										<div id="groupBloc" class="form-group row">
											<label class="col-md-3 col-form-label{{ $groupIdError }}">Product / Service Group </label>
											<div class="col-md-8">
												<select name="group_id" id="groupId" class="form-control selecter{{ $groupIdError }}">
													<option value="0"
															@if (old('group_id', $post->group_id)=='' or old('group_id', $post->group_id)==0)
																selected="selected"
															@endif
													> Select a Product / Service group</option>

													@foreach ($groups as $group)
														<option value="{{ $group->id }}"
																@if (old('group_id', $post->group_id)==$group->id)
																	selected="selected"
																@endif
														> {{ $group->name }} </option>
													@endforeach
												...
												<small id="" class="form-text text-muted"><a href="javascript:void(0)" onclick="showGroup()">Create new group</a></small>
											</div>
										</div>

										<div id="groupCreateBloc" class="form-group row hide">
											<label class="col-md-3">&nbsp;
											</label>
											<div class="col-md-8 ">
												<input id="group_name" name="group_name" placeholder="Group Name" class="form-control input-md"
													   type="text" value="">
												<small id="group_name_error" class="form-text hide" style="color:#dc3545"></small>

													   <br>
												 <button type="button" onclick="saveGroup()">save</button> <button type="button" onclick="cancelGroup()">cancel</button>
											</div>
										</div>
										<!-- title -->
										<?php $titleError = (isset($errors) and $errors->has('title')) ? ' is-invalid' : ''; ?>
										<div class="form-group row required">
											<label class="col-md-3 col-form-label" for="title">Product / Service Name  <sup>*</sup></label>
											<div class="col-md-8">
												<input id="title" name="title" placeholder="Product / Service Name " class="form-control input-md{{ $titleError }}"
										...
										<?php $shortDescriptionError = (isset($errors) and $errors->has('short_description')) ? ' is-invalid' : ''; ?>
										<div class="form-group row required">
											...
											<div class="col-md-8">
												...
												<textarea
														class="form-control{{$shortDescriptionError }}"
										...
										--}}
										<?php
											$contactNameError = (isset($errors) and $errors->has('contact_name')) ? ' is-invalid' : '';
											$emailError = (isset($errors) and $errors->has('email')) ? ' is-invalid' : '';
										...
										<div class="form-group row">
											<label class="col-md-3 col-form-label{{ $filenameError }}" for="tags">Brochure</label>
											...
											<div class="col-md-8">
												...
												@if($post->brochure)
											<label ><a href="{{ lurl('storage/' . $post->brochure) }}" target="_blank">Current</a>  | <a href="javascript:void(0)" onclick="deleteBrochure('{{$post->id}}')">Delete</a></label>
											@endif
											</div>
										</div>

                                                                               <div class="form-group row">
											<label class="col-md-3 col-form-label{{ $filenameError }}" for="tags">Yout Tube Video Link</label>

											<div class="col-md-8">
												<input id="filename" name="video" type="text" class="form-control"  value="{{ old('video_link', $post->video_link) }}">
...
@section('after_scripts')
	...
	<script>
		...
		var lang = {
			'select': {
				'category': "Select a Pharmafranchisemart category",
				'subCategory': "Select a Pharmafranchisemart sub-category",
			...
			if($('#group_name').val()){
				...
				$.ajax({
					...
					if(data.error){
						...
						$('#group_name_error').html(data.error.message).show();
					}
					else{
						$('#groupId')
				          .append($('<option>', { value : data.id,selected:'selected' })
				          .text(data.name));
				...
				$('#group_name').addClass('is-invalid');
				$('#group_name_error').html('Group Name cannot be blank.').show();
			}


		}
		function showGroup(){
			$('#groupCreateBloc').removeClass('hide');
		}
		function cancelGroup(){
			$('#group_name').val("");
			$('#group_name').removeClass('is-invalid');
			$('#group_name_error').hide();
			$('#groupCreateBloc').addClass('hide')
		}
	...
	<script>
		...
		function deleteBrochure(id){
			...
			if (confirm('Are you sure you want to delete this brochure?')) {
				window.location.href="{{ lurl('post/deletebrochure') }}/"+id
			}
		}
